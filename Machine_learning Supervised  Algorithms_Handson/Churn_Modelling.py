...
Path1 = "Churn_Modelling.csv"
df = pd.read_csv(Path1)
print("DataSet Loaded ")
X = df.iloc[:,3:12].values #Labels 
...
onehotencode = OneHotEncoder(categorical_features = [1,2])
...
def validation_curve_model(X, Y, model, param_name, parameters, cv,x_label,fig_name):
    ...
    plt.fill_between(parameters, train_scores_mean - train_scores_std,train_scores_mean + train_scores_std,             alpha=0.1,color="r")
...
def Svm_fun(option):
    ...
    y_pred = clf.predict(X_test)
    cm = confusion_matrix(y_test,y_pred)
    Pred = (cm[0,0]+cm[1,1])/2000
    print("Prdectied Accuracy SVM linear kernel classifier:{}".format(Pred))
    param_range = np.linspace(0.1, 10, 10)
    validation_curve_model(X, y, model=SVC(kernel='poly'), param_name="C", parameters=param_range, cv=4,x_label="itrations",fig_name='SVMVpoly.jpg')
...
def ADAboost_fun(option):
    ...
    validation_curve_model(X, y, AdaBoostClassifier(DecisionTreeClassifier(max_depth=5, min_samples_leaf=5)),
                           param_name="n_estimators",parameters=param_range, cv=4, x_label="Esimators",fig_name='ADA.jpg')
    plt.show()
    train_size=np.linspace(.1, 1.0, 15)
